...
class JsonSchemaModule extends AbstractModule {
  ...
  */
  async getSchema(baseSchemaName) {
    ...
    const schema = modelSchemas.reduce((m,p) => jsonMergePatch.apply(m,p.$merge.with), modelSchemas.shift());
    extensionSchemas.forEach(s => {
      if(s.$merge) jsonMergePatch.apply(schema, s.$merge.with);
      else this.log('warn', `cannot apply '${s.$id}' merge schema to ${schema.$id}, invalid schema format`, s);
    });
    return schema;
  }
