...
class JsonSchemaModule extends AbstractModule {
  ...
  constructor(app, pkg) {
    ...
    */
    this.schemaPaths = {};
    /**
    * Metadata stored about registered schemas
    * @type {Object}
    */
    this.schemaMetadata = {};
    /**
  ...
  */
  async registerSchema(filePath) {
    ...
    this.storeSchemaMetadata(name, json);

    this.log('debug', `registered schema '${name}' at ${filePath}`);

    if(json.$merge && json.$merge.type === 'merge') {
      this.extendSchema(json.$merge.source.$ref, name);
    }
  ...
  */
  async getSchema(schemaName) {
    ...
    return modelSchemas.reduce(async (m,s) => this.applyPatch(m,s), modelSchemas.shift());
  }
  async loadSchemaHierarchy(schemaName, schemas=[]) {
    ...
    const extensions = await this.loadSchemaExtensions(schema.$id);
  ...
  applyPatch(sourceSchema, patchSchema, options = { extendAnnotations: true, overwriteProperties: false }) {
    ...
    if(mergeData.properties) {
      ...
      func(sourceSchema.properties, _.cloneDeep(mergeData.properties));
    }
  ...
  */
  async loadSchema(schemaName) {
    ...
    try {
      const schema = await fs.readJson(this.schemaPaths[schemaName]);
      this.storeSchemaMetadata(schemaName, schema);
      return schema;
    } catch(e) {
  ...
  */
  ...
  /**
  * Stores metadata related to a schema
  * @param {String} schemaName The name of the schema (as registered with the module)
  * @param {Object} schema The JSON schema object
  */
  storeSchemaMetadata(schemaName, schema) {
    const props = schema.$merge ? schema.$merge.with.properties : schema.properties;
    if (!props) {
      return this.log('error', `couldn't determine properties for schema '${schemaName}'`);
    }
    const storeKeys = (o) => {
      Object.entries(o).forEach(([k, { format, isObjectId, properties, type }]) => {
        if (type === 'object' && properties) {
          return storeKeys(properties);
        }
        if (type === 'string' && format) {
          type = `${type}:${format}`;
        }
        if (type === 'string' && isObjectId) {
          type = `${type}:objectid`;
        }
        if (!this.schemaMetadata[type]) this.schemaMetadata[type] = [];
        this.schemaMetadata[type].push(k);
      });
    }
    storeKeys(props);
  }
