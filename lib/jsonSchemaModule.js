...
class JsonSchemaModule extends AbstractModule {
  ...
  async registerSchema(filePath, options = {}) {
    ...
    const schema = await (new JsonSchema({
      filePath,
      validator: this.validator,
      enableCache: this.getConfig('enableCache'),
      cacheLifespan: this.getConfig('cacheLifespan')
    })).load(options);

    if(this.schemaPaths[schema.name]) {
      if(options.replace) this.deregisterSchema(schema.name);
      else throw this.app.errors.SCHEMA_EXISTS.setData({ name: schema.name, filepath });
    }
    this.schemas[schema.name] = schema;
    this.log('debug', 'REGISTER_SCHEMA', schema.name, filePath);
    
    if(json.$patch) this.extendSchema(json.$patch?.source?.$ref, schema.name);
  }
  /**
  ...
  createSchema(filePath, options) {
    return (new JsonSchema(filePath, options)).load();
  }
  /**
  ...
  extendSchema(baseSchemaName, extSchemaName) {
    ...
    if(!baseSchema) {
      throw this.app.errors.NOT_FOUND.setData({ type: schema, id: schemaName });
    }
  ...
  async getSchema(schemaName, options = {}) {
    if(!_.isString(schemaName)) throw this.app.errors.INVALID_PARAMS.setData({ params: ['schemaName'], value: schemaName });
    if(!this.schemaPaths[schemaName]) throw this.app.errors.NOT_FOUND.setData({ type: 'schema', id: schemaName });

    const cache = this.schemaCache.get(schemaName)?.data;
    if(options.useCache && cache) return cache;
    
    return this.createSchema(this.schemaPaths[schemaName], options);
  }
}
