...
class JsonSchemaModule extends AbstractModule {
   ...
   * @param {Object} options Extra options
   * @param {Boolean} options.useCache Whether cached schemas should be used
   * @return {Promise}
   */
  async loadSchema(schemaName, options = {}) {
    ...
    try {
      const schema = JSON.parse((await fs.readFile(this.schemaPaths[schemaName])).toString());
   ...
   * @param {Object} options The name of the schema
   * @param {Boolean} options.useCache Whether cached schemas should be returned
   * @return {Promise} Resolves with an array of the extension schemas
   */
  async loadSchemaExtensions(schemaName, options = {}) {
    ...
    await Promise.all(this.schemaExtensions[schemaName].map(async s => {
      try {
        extensionSchemas.push(await this.loadSchema(s, { useCache: options.useCache }));
      } catch(e) {
  ...
  applyPatch(sourceSchema, patchSchema, options = {}) {
    ...
    if(patchData.properties) {
      ...
      Object.mergeFunc(sourceSchema.properties, patchData.properties);
    }
   ...
   * @param {Boolean} options.useCache Whether cached should be returned 
   * @return {Promise} Resolves with the schema
   */
  async getSchema(schemaName, options = {}) {
    const { applyExtensions, extensionFilter } = options;
    let schema = await this.loadSchema(schemaName, { useCache: options.useCache });
    const mergeRef = schema?.$merge?.source?.$ref;
    if(!mergeRef) { // always start from the base schema
      this.applyPatch(schema, await this.loadSchema(JsonSchemaModule.BASE_SCHEMA_NAME), { strict: false, extendAnnotations: false });
    } else { // merge parent schema
      schema = this.applyPatch(await this.getSchema(mergeRef, options), schema);
    }
    const extensions = await this.loadSchemaExtensions(schemaName, { useCache: options.useCache });
    await Promise.all(extensions.map(async s => {
      const applyPatch = typeof extensionFilter === 'function' ? extensionFilter(s.$anchor) : applyExtensions;
