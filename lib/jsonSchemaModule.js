  ...
  async registerSchemas() {
    ...
    return Promise.all(Object.values(this.app.dependencies).map(d => {
      return new Promise((resolve, reject) => {
        glob(globPattern, { cwd: d.rootDir }, (error, files) => {
          if(error) return reject(error);
          if(!files.length) return resolve();
          files.forEach(f => this.schemaPaths[f.match(re)[1]] = path.join(d.rootDir, f));
          resolve();
        });
      });
    }));
  ...
  */
  validate(schemaName, dataToValidate) {
    return new Promise(async (resolve, reject) => {
      let validate;
      try {
        const schema = Utils.isString() ? await this.getSchema(schemaName) : schemaName;
