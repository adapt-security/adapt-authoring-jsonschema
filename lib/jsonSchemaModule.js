  ...
  */
  async registerSchema(filePath) {
    try {
      ...
      if(!file.title) {
        return this.log('error', `Failed to register schema at ${filePath}, invalid title '${file.title}'`);
      }
      this.schemaPaths[file.title] = filePath;
      this.log('debug', `registered schema '${file.title}' from ${filePath.replace(this.app.getConfig('root_dir'), '')}`);
    } catch(e) {
      this.log('error', `Failed to register schema at ${filePath}, ${e}`);
    }
  }
  /**
