...
class JsonSchemaModule extends AbstractModule {
  ...
  async registerSchemas() {
    ...
    return Promise.all(Object.values(this.app.dependencies).map(d => {
      return new Promise((resolve, reject) => {
        glob('schema/*.schema.json', { cwd: d.rootDir }, (error, files) => {
          if(error) return reject(error);
          if(!files.length) return resolve();
          files.forEach(f => this.registerSchema(path.join(d.rootDir, f)));
          resolve();
        });
      });
    }));
  ...
  * @param {String|Object} filePath The path to the schema file, or the schema contents
  * @return {Promise}
  */
  async registerSchema(filePath) {
    const isLoaded = Utils.isObject(filePath);
    try {
      const file = isLoaded ? filePath : await fs.readJson(filePath);
      if(!file.title) {
        throw new Error(`schema has invalid title value '${file.title}'`);
      }
      this.schemaPaths[file.title] = filePath;
      this.log('debug', `registered schema '${file.title}' from ${isLoaded ? 'object' : 'file'}`);
    } catch(e) {
      this.log('error', `failed to register schema '${file.title}'`);
      this.log('error', e);
      return;
    }
  }
  ...
  validate(schemaName, dataToValidate) {
    return new Promise(async (resolve, reject) => {
      ...
      try {
        ...
        validate = await this.validator.compileAsync(schema);
      } catch(e) {
        return reject(e);
      }
      ...
      if(!isValid) {
        ...
        return reject(new DataValidationError('Data validation failed', errors));
      }
      resolve();
    });
  }
