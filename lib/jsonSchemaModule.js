...
class JsonSchemaModule extends AbstractModule {
   ...
   * @param {Boolean} options.replace Replace existing schema
  ...
  async registerSchema(filePath, options = {}) {
    ...
    if(this.schemaPaths[name] && !options.replace) {
      return this.log('error', `cannot register schema, name '${name}' is in use, ${filePath}`);
    }
  ...
  async getSchema(schemaName, applyExtensions = true) {
    const schemas = await this.loadSchemaHierarchy(schemaName);
    return applyExtensions ? 
      schemas.reduce((m,s) => this.applyPatch(m,s), schemas.shift()) :
      schemas.pop();
  }
  /**
  ...
  async loadSchema(schemaName) {
    ...
    if(!this.schemaPaths[schemaName]) {
      ...
      try {
        const baseSchema = JSON.parse((await fs.readFile(this.schemaPaths['base'])).toString());
        const schema = JSON.parse((await fs.readFile(this.schemaPaths[schemaName])).toString());
        schema.properties = _.assign(baseSchema.properties, schema.properties);
        return schema;
      } catch(e) {
