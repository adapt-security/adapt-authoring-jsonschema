...
class JsonSchemaModule extends AbstractModule {
  ...
  async init() {
    ...
    this.validator = new Ajv({
      ...
      strict: false,
      verbose: true,
      // the following keywords will filter data during sanitise
      keywords: ['isInternal', 'isReadOnly'].map(k => {
        return {
          keyword: k,
          type: 'string',
          modifying: true,
          schemaType: 'boolean',
          compile: function(schema, parentSchema, context) {
    ...
    this.onReady().then(() => { // we don't want to wait for these tasks before setting ready status
      this.initFormats();
  ...
  async sanitise(schemaName, dataToSanitise, options) {
    const opts = {
      useDefaults: false,
      ignoreRequired: true,
    ...
    return this.validator.serialize(schemaName, dataToSanitise, opts);
  }
}
