...
class JsonSchemaModule extends AbstractModule {
  ...
  */
  async registerSchema(filePath) {
    const shortPath = filePath.replace(path.join(this.app.getConfig('root_dir'), 'node_modules', path.sep), '');
    try {
      ...
      this.log('debug', `registered schema from ${shortPath}`);
    } catch(e) {
      this.log('error', `failed to register schema from ${shortPath}`);
      this.log('error', e);
    }
  }
  /**
