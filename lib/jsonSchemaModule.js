...
class JsonSchemaModule extends AbstractModule {
  ...
  async loadSchemaHierarchy(schemaName, schemas=[]) {
    ...
    if(schema.$anchor) {
      // apply the base schema properties
      const props = schema.properties || schema.$merge?.with || schema.$patch?.with;
      const baseSchema = await this.loadSchema('base');
      Object.assign(props, baseSchema.properties);
  ...
  async applyDefaults(schema, data={}) {
    try {
      const hierarchy = await this.loadSchemaHierarchy(schema);
      return hierarchy.reduce((m, s) => this.getObjectDefaults(s, m), data);
    } catch(e) {
  ...
  async validate(schemaName, dataToValidate, options = {}) {
    ...
    const data = _.merge({}, dataToValidate, o.useDefaults ? await this.applyDefaults(schema) : {});
