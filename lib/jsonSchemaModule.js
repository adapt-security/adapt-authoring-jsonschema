...
class JsonSchemaModule extends AbstractModule {
  ...
  */
  getSchema(schemaName) {
    ...
    const schemaExtensions = this.schemaExtensions[schemaName].map(d => {
      try {
        return require(this.schemaPaths[d]);
      } catch(e) {
        throw new Error(`Failed to load '${schemaName}' schema extension at ${d}, ${e.message}`);
      }
    });
