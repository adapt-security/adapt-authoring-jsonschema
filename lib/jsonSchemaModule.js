...
class JsonSchemaModule extends AbstractModule {
  ...
  async initRoutes() {
    ...
    */
    ...
    this.router.addRoute({
      route: '/',
      handlers: { get: this.handleRoot.bind(this) }
    }, {
  ...
  */
  async registerSchema(filePath) {
    ...
    const name = filePath.match(/\W(\w+)\.schema\.json/)[1];
  ...
  */
  ...
  handleRoot(req, res, next) {
    res.send(Object.keys(this.schemaPaths).reduce((m,k) => {
      m[`${k}_schema_url`] = `${this.router.url}/${k}.schema.json`;
      return m;
    }, {}));
  }
  /**
