...
class JsonSchemaModule extends AbstractModule {
   ...
   * @param {Boolean} options.useCache If false, cache data Will be ignored
   * @return {Promise} Resolves with the schema
   */
  async getSchema(schemaName, options = {}) {
    const { applyExtensions, extensionFilter } = options;
    // return cached data if it exists and is enabled
    const useCache = this.getConfig('enableCache') && options.useCache !== false;
    const cacheKey = `${schemaName}-${JSON.stringify(options)}`;
    const cache = this.schemaCache?.[cacheKey];
    if(useCache && cache && Date.now() <= (cache.timestamp + this.getConfig('cacheLifespan'))) {
      return cache.schema;
    }
    let schema = await this.loadSchema(schemaName);
    ...
    if(useCache) {
      this.schemaCache[cacheKey] = { schema, timestamp: Date.now() };
    }
    return schema;
  }
  /**
