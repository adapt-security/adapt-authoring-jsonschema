...
class JsonSchemaModule extends AbstractModule {
  ...
  async getSchema(schemaName) {
    ...
    return modelSchemas.reduce((m,s) => this.applyPatch(m,s), modelSchemas.shift());
  }
  /**
  ...
  applyPatch(sourceSchema, patchSchema, options = { extendAnnotations: true, overwriteProperties: false }) {
    ...
    try {
      ...
      this.log('warn', `cannot apply '${patchSchema.$anchor}' merge schema to ${sourceSchema.$anchor}, invalid schema format`, patchSchema);
      return sourceSchema;
    }
  ...
  async validate(schemaName, dataToValidate, options = {}) {
    console.log('JsonSchemaModule#validate', schemaName);
