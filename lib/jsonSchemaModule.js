...
class JsonSchemaModule extends AbstractModule {
  ...
  */
  async registerSchema(filePath) {
    ...
    /*
    if(json.$merge) {
      if(!json.$merge.source.$ref) {
        this.log('error', `must specify '$ref' for $merge schema '${name}'`);
        return;
      }
  ...
  */
  ...
  /**
  * Like getSchema, but returns collection of schemas
  ...
  * @return {Promise} Resolves with the relavent schemas as an object
  * @example
  * {
  *   course: { .. },
  *   courseExtension: { ... }
  ...
  */
  async composeSchema(baseSchemaName, ...additionalSchemaNames) {
    const schemas = {
      [baseSchemaName]: await this.loadSchema(baseSchemaName)
    };
    ...
    await Promise.all(additionalSchemaNames.map(async s => {
      try {
        schemas[s] = await this.loadSchema(s);
      } catch(e) {
  ...
  */
  async applyDefaults(schemaName, data={}) {
    ...
    try {
      return (await getSchemaChain(schemaName)).reduce((m,s) => {
  ...
  */
  async validate(schemaName, dataToValidate, o = {}) {
    const options = _.defaults(o, { useDefaults: true, ignoreRequired: false });
    const data = _.defaultsDeep(options.useDefaults ? this.applyDefaults() : {}, dataToValidate);
