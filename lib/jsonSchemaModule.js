...
class JsonSchemaModule extends AbstractModule {
  ...
  */
  ...
  boot(app, resolve, reject) {
    Object.keys(this.schemas).forEach(k => {
      try {
        this.validator.addSchema(this.getSchema(k),k);
      } catch(e) {
        console.log(e);
      }
    });
  ...
  */
  addSchema(schemaData) {
    ...
    if(!isValid) {
      this.log('error', `Cannot add invalid schema '${schemaData.title}', ${this.validator.errors.map(e => `${e.schemaPath} ${e.message}`)}`);
      return;
    }
  ...
  */
  extendSchema(schemaName, schemaData) {
