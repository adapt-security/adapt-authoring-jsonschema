...
class JsonSchemaModule extends AbstractModule {
  ...
  */
  async validate(schemaName, dataToValidate, options = {}) {
    options = Object.assign({ useDefaults: true, ignoreRequired: false }, options);
    ...
    if(validate(dataToValidate)) {
      return; // validated ok, nothing to do
    }
    const errors = [];
