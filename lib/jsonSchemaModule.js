...
class JsonSchemaModule extends AbstractModule {
  ...
  async loadSchema(schemaName, options) {
    ...
    const compiled = await this.validator.compileAsync(schema);
  ...
  applyPatch(sourceSchema, patchSchema, options = {}) {
    ...
    ['allOf','anyOf','oneOf'].forEach(p => {
      if(patchData[p]?.length) sourceSchema[p] = (sourceSchema[p] ?? []).concat(patchData[p]);
    });
  ...
  async getSchema(schemaName, options = {}) {
    ...
    const schema = (options.useCache !== false && this.schemaCache.get(schemaName)) || 
  ...
  async validate(validateFunc, dataToValidate, options = {}) {
    ...
    if(!validateFunc(data)) {
      ...
      if(errors.length) throw this.app.errors.VALIDATION_FAILED.setData({ schemaName: schema.$anchor, errors, data })
    }
    return data;
  }
  /**
