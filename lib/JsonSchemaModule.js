...
import { Schemas, Schema, SchemaError, XSSDefaults } from 'adapt-schemas'
...
class JsonSchemaModule extends AbstractModule {
  ...
  async init () {
    ...
    // Create directory replacements for the Keywords from app config
    const directoryReplacements = {
      $ROOT: this.app.rootDir,
      $DATA: this.app.getConfig('dataDir'),
      $TEMP: this.app.getConfig('tempDir')
    }
