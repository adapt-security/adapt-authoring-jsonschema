...
class JsonSchemaModule extends AbstractModule {
  ...
  async getSchema (schemaName, options = {}) {
    ...
    /*
      TODO: record construction time of cached schemas
      using a debounced function check all cached schema ages and purge those older than [maxAge]
      this simple method will reduce memory load if usage becomes lighter
    */

    const cacheId = options.getCacheId?.()
    if (cacheId) {
      let subcache = this.cache[cacheId]

      if (!subcache) this.cache[cacheId] = subcache = {}

      let cachedSchema = subcache[schemaName]

      if (!cachedSchema || (options.useCache === false && !cachedSchema?.isBuilding)) {
        console.log(schemaName, 'created for', cacheId, Object.keys(subcache).length)
        cachedSchema = new JsonSchema({
          enableCache: this.getConfig('enableCache'),
        ...
        const { extensions, raw, name } = schema
        Object.assign(cachedSchema, { extensions, raw, name })

        subcache[schemaName] = cachedSchema

        await options.createCallback?.()
      }

      return cachedSchema.build(options)
    }
