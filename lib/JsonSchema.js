...
class JsonSchema {
  ...
  async getParent() {
    ...
    return jsonschema.getSchema(mergeRef ?? BASE_SCHEMA_NAME);
  }
  /**
   * 
   * @param {*} options 
  ...
  async build(options = {}) {
    if(this.enableCache && await this.isBuildValid()) {
      return this;
    }
    ...
    while(parent) {
      await this.patch(this.built, parent.raw);
    ...
    if(this.extensions.length) {
      await Promise.all(this.extensions.map(async s => {
        ...
        if(applyPatch) {
          ...
          this.patch(this.built, extSchema.raw, { extendAnnotations: false });
        }
      }));
    }
  ...
  async sanitise(dataToSanitise, options = {}) {
    ...
    return _.mapValues(this.built.properties, (config, prop) => {
      ...
      return config.type === 'object' && config.properties ? this.sanitise(config, value, opts) : 
        config.type === 'string' && opts.sanitiseHtml ? xss(value, this.xssWhitelist) :
        value;
    });
  }
  /**
