...
class JsonSchema {
  constructor({ enableCache, filePath, validator, xssWhitelist }) {
    ...
    /**
     * Attributes whitelisted for XSS sanitisation
     * @type {Object}
     */
  ...
  async sanitise(dataToSanitise, options = {}) {
    const defaults = { isInternal: false, isReadOnly: false, sanitiseHtml: true, strict: true };
    return this.sanitiseRecursive((await this.build()).built, dataToSanitise, _.defaults(options, defaults));
  }
  async sanitiseRecursive(schema, data, opts) {
    const sanitised = {};
    for (const prop in schema.properties) {
      ...
      if(ignore) {
        if(opts.strict) throw App.instance.errors.MODIFY_PROTECTED_ATTR.setData({ attribute: prop, value: value });
        continue;
      }
      sanitised[prop] = 
        schemaData.type === 'object' && schemaData.properties ? await this.sanitiseRecursive(schemaData, value, opts) : 
        schemaData.type === 'string' && opts.sanitiseHtml ? xss(value, this.xssWhitelist) :
        value;
    }
    return sanitised;
  }
