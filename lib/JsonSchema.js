...
class JsonSchema {
  ...
  async load(options = {}) {
    ...
    let schema, name;
    try {
      schema = JSON.parse((await fs.readFile(this.schemaPath)).toString());
      name = json.$anchor;
    } catch(e) {
    ...
    if(mergeRef) { // merge parent schema
      const parentSchema = await jsonschema.getSchema(mergeRef, { ...options, compiled: false });
      schema = this.patch(parentSchema, schema);
    } else if(schema?.$anchor !== BASE_SCHEMA_NAME) { // extend all schemas from the base schema
      baseSchema = await jsonschema.getSchema(BASE_SCHEMA_NAME, { compiled: false })
      this.patch(baseSchema, { strict: false, extendAnnotations: false });
    }
    if(jsonschema.schemaExtensions[schemaName]) {
      await Promise.all(jsonschema.schemaExtensions[schemaName].map(async s => {
        const { schema: extSchema } = await jsonschema.getSchema(s, { useCache: options.useCache });
        const applyPatch = typeof extensionFilter === 'function' ? extensionFilter(extSchema.$anchor) : applyExtensions !== false;
        if(applyPatch) this.patch(schema, extSchema, { extendAnnotations: false });
      }));
    }
    ...
    this.name = schema.$anchor;
    this.raw = schema;
    this.compiled = await this.compile(schema);
